...
このリポジトリは、日本の就職活動と転職のための可搬な Agent Skills を管理する。キャリア相談、応募書類の添削、面接対策、企業研究、内定判断、おためし転職の体験・業務設計、副業の可否と案件条件の確認を対象とする。**公開リポジトリであり、求職者本人が自分の Claude にインストールして使う。`trial` カテゴリでは求人企業の採用担当者も対象とし、それぞれの立場で使うスキルを分ける。**

## 構成

- スキルは `skills/<category>/<skill-name>/SKILL.md` に置く。
- カテゴリは `career`、`documents`、`interview`、`research`、`offer`、`trial`、`sidework` のいずれか。定義は `scripts/_repo.py` の `CATEGORIES` だけにあるので、新設するときはそこを更新する。
