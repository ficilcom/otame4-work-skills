...
このリポジトリは、日本の就職活動と転職のための可搬な Agent Skills を管理する。キャリア相談、応募書類の添削、面接対策、企業研究、内定判断、おためし転職の体験・業務設計を対象とする。**公開リポジトリであり、求職者本人が自分の Claude にインストールして使う。`trial` カテゴリでは求人企業の採用担当者も対象とし、それぞれの立場で使うスキルを分ける。**
...
- **複数の段階を1スキルに束ねてよいのは、次を両方満たすときだけ。** `trial` の2スキルがこれにあたる。
...
- **立場が違うものは必ず分ける。** 求職者向けと採用担当者向けは、同じ段階でも守るべき制約が違う（`otameshi-candidate` と `otameshi-employer`）。
...
`trial` の初版は相談・原稿作成・運用準備のみを行う。企業側から候補者への連絡も含め、掲載・応募・送信・日程確定などの外部操作は利用者本人が行う。
