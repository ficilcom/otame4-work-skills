...
このリポジトリのスキルは、就職活動と転職で繰り返し発生する作業を、再利用できる手順に変えるためのものである。一般論のアドバイスではなく、判断を変える情報、失敗を防ぐ制約、使える成果物を持たせる。利用者は求職者本人であり、`trial` カテゴリでは求人企業の採用担当者も対象とする。スキルは公開され、立場ごとに各自の環境にインストールされる。
...
| `trial` | おためし転職の応募・募集、無償体験と有償業務の設計、条件調整、振り返り |
...
- 求職者、または `trial` の求人企業の具体的で反復する作業を解いている。
