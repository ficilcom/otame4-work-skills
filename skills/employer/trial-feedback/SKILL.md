...
3. `python3 scripts/review_trial.py <input.json>` で、事前の基準ごとの結果と観察数、後から足した期待、支援の遅れが関わる未達、実施済みの実働に対する精算額、フィードバック案に混ぜてはいけないものを機械的に出す。入力形式は [振り返りの整理の仕方](references/debrief-method.md#スクリプト入力) を見る。**精算は評価と分けて数える。**
