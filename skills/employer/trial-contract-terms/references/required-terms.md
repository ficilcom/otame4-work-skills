...
- **成果への満足や採用の判断を支払いの条件にしない。** 実施済みの実働と検収済みの給付に対する支払いは、評価や採用判断と分ける。「期待に届かなかったので減額」は、検収の基準が書面にあり、それに沿って行う場合しか成り立たない。成果で支払いを判断するつもりなら、`inspection`（誰が、何を見て、いつまでに）は着手前に書く。
...
- `document.form` は上の出典表の値、または `none`（候補者にまだ何も伝えていない）。**チャットや口頭で伝えた条件があるなら `chat` / `verbal` にする。** `platform` / `chat` / `verbal` / `none` は書面でない扱いになる。
- `items` は確認した項目だけを入れる。入れなかった項目は「未確認」として数えられる。`status` は `stated` / `missing`（書いていないことを確認した）/ `unclear`（書いてはあるが読み取れない）/ `unknown`（未確認）。**未確認を `missing` に丸めない。** `source` は、`stated` ならその条件の出所、`missing` / `unclear` ならどの文書を見てそう判断したかを入れる。条件付きの項目は `applicable` で対象かどうかを入れる。`payer` は、どこにも書いていないと確認したなら `missing`、支払主体そのものをサービスの規約で確認できていないなら `unknown` にし、`note` に何が未確認かを書く。
...
- `payment.reference_term_days` は、受領日から支払期日までの日数として参照する上限で、**一次情報で確認した値だけを入れる。** 入れなければ比較しない。比較は受領日起点の日数で行い、検査完了日起点の日数は参考として出すだけである。起点をどちらにするかの定めも公表資料で確認する。
- `revisions` は修正の回数と時間の上限。`rounds` の `0` は「修正なしで合意した」であり、時間の上限がなくても範囲は閉じている。`null` は「決めていない」で別物である。`unpaid` は修正を無償にしているか。
