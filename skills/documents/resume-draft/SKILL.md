...
1. **素材がそろっているかを先に確かめる。** 在籍期間と、期間ごとの経験（状況・行動・結果・役割・裏づけ）が必要である。`career-inventory` の結果があればそれを使う。なければ、在籍期間を先に並べてもらい、経験が薄い期間は `career-inventory` で洗い出してから戻ることを勧める。**素材が足りない箇所を文章で埋めない。**
