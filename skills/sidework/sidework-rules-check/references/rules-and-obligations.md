...
| `application_procedure` | 申請・届出の様式、提出先、時期 | `procedure` | 許可制・届出制のとき |
| `approval_criteria` | 許可・受理の判断基準 | `procedure` | 許可制のとき |
| `revocation_conditions` | 許可の取消・変更の条件 | `procedure` | 許可制のとき |
| `ongoing_report` | 開始後の報告義務（稼働時間、収入、内容の変更） | `procedure` | 許可制・届出制のとき |
...
| `reputation_clause` | 会社の信用・名称・肩書の使用に関する定め | `duty` | 常に |
...
対象になる条件が書かれた項目は、その条件が確認できるまで対象外にしない。規定の型、立場、副業の型が未確認のうちは、対象かどうかを判定せず未確認のまま残す（スクリプトは `applicable` を `null` で返す）。**確認していないことを、対象外と同じに扱わない。**
...
| 信用 | 勤務先の名前や在籍を出して活動するか（`reputation_clause`） |
| 健康と安全 | 本業と合わせた稼働時間、休みの日、深夜の作業 |

肩書や在籍を出すかどうかは、本人が思っているより広く取られることがある。SNSの表示、登壇のプロフィール、請求書の記載も同じ論点に入る。見るのは会社の信用・名称の使用に関する定めであって、職務に関する成果物・発明の条項ではない。
...
- **「届出」と書いてあっても、受理されなければ始められない運用のことがある。** 様式に「承認欄」があるかで運用が読めることがある。書面と運用が違いそうなときは、そのまま質問にする。届出制として読んだ結果、許可の判断基準と取消の条件は確認の対象から外れるため、この読み違いは確認の抜けになる。
...
- `application` は、許可制・届出制のときだけ数える。禁止・規定なしでは手続きがないため対象外にし、規定の型が未確認のときは要否も決まらないため対象として残す。`code` は `counterparty`（副業先）、`work_content`（業務の内容）、`engagement_form`（契約の形態）、`schedule`（曜日と時間帯）、`weekly_hours`（週の稼働）、`period`（開始日と期間）、`income`（見込みの報酬）、`conflict_statement`（競業・秘密保持に触れない説明）、`impact_plan`（本業への影響を避ける具体策）。`status` は `ready` / `draft` / `missing`。
